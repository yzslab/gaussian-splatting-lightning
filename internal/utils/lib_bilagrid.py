...
def _num_tensor_elems(t): return max(torch.prod(torch.tensor(t.size()[1:]).float()), 1.)
...
class BilateralGridCP4D(nn.Module):
    ...
    def __init__(self,
        ...
        if self.learn_gray:
            def rgb2gray_mlp_linear(l): return nn.Linear(self.gray_mlp_width, self.gray_mlp_width if l < self.gray_mlp_depth - 1 else 1)
            def rgb2gray_mlp_actfn(_): return nn.ReLU(inplace=True)
