...
class ScaleRegularizationMetricsModuleMixin:
    def get_scale_regularization_metrics(self, gaussian_model, metrics, pbar):
        ...
        over_scales = max_scales - self.config.max_scale
        is_over_scales = over_scales.detach() > 0.
        n_over_scales = is_over_scales.sum()

        scale_ratios = max_scales / (mid_scales + 1e-8)
        over_ratios = scale_ratios - self.config.max_scale_ratio
        is_over_ratios = over_ratios.detach() > 0.
        n_over_ratios = is_over_ratios.sum()

        over_scale_loss = (over_scales * is_over_scales).sum() / (n_over_scales + 1) * self.config.scale_reg_lambda
        over_ratio_loss = (over_ratios * is_over_ratios).sum() / (n_over_ratios + 1) * self.config.scale_ratio_reg_lambda
        ...
        return sorted_scales, over_scales, is_over_scales, over_ratios, is_over_ratios
