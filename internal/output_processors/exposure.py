...
class ExposureProcessorModule(torch.nn.Module):
    ...
    def training_setup(self, pl_module):
        ...
        exposures = torch.full(
            (n_appearances, 4),
            inverse_sigmoid(torch.tensor(1., dtype=torch.float32)),
            dtype=torch.float32,
            device=pl_module.device,
        )
        exposures[:, -1] = inverse_sigmoid(torch.tensor(0.1, dtype=torch.float32, device=pl_module.device))
        assert torch.all(torch.sigmoid(exposures[:, :3]) == 1.)
        assert torch.all(torch.sigmoid(exposures[:, -1]) == 0.1)
    ...
    def training_forward(self, batch, outputs):
        ...
        rendered_image = adjustment[:3, None, None] * rendered_image
        rendered_image = torch.pow(rendered_image + 1e-5, adjustment[-1] * 10)
