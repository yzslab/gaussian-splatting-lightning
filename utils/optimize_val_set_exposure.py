...
class ExposureOptimizer(lightning.LightningModule):
    def __init__(
            self,
            config: Config,
            ckpt,
        ...
        self.val_output_dir = os.path.join(self.config.model, "exposure-val", "on_{}".format(self.config.optimize_on))
    ...
    def setup(self, stage: str) -> None:
        ...
        output_processor = GaussianModelLoader.initialize_output_processor_from_checkpoint(
            ckpt,
            stage,
            self.device,
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.trainer.current_epoch + 1 >= self.trainer.max_epochs:
            image_output_path = os.path.join(self.val_output_dir, "{}.jpg".format(image_name.replace("/", "_")))
            ...
            self.val_metrics.append((image_name, metrics))
    ...
    def on_validation_epoch_end(self, name="val") -> None:
        ...
        if len(self.val_metrics) > 0:
            ...
            with open(os.path.join(self.val_output_dir, f"metrics.csv"), "w") as f:
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(
            ...
            params=self.output_processor.parameters(),
        )
...
def main():
    ...
    exposure_optimizer = ExposureOptimizer(config, ckpt)
    exposure_optimizer.setup("fit")
    ...
    output_dir = os.path.join(os.path.dirname(os.path.dirname(ckpt_file)), "exposure_optimization")
    ...
    if config.val_only:
        trainer.validate(exposure_optimizer, dataloaders=dataloader)
        return
    else:
        # start fitting
        trainer.fit(exposure_optimizer, train_dataloaders=dataloader, val_dataloaders=dataloader)

    # save
    if trainer.global_rank == 0:
        exposure_state_dict = exposure_optimizer.output_processor.state_dict()
        ckpt = torch.load(ckpt_file, map_location="cpu")
        prefix = "output_processor."
        for k in exposure_state_dict:
            key = "{}{}".format(prefix, k)
            assert key in ckpt["state_dict"]
            ckpt["state_dict"][key] = exposure_state_dict[k]
